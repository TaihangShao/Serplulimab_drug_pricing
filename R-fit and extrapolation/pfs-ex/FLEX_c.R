...
modFP2_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(1)) + I(Time^2) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^0.5) + I(Time^3) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
...
write.csv(dfGOF,"che-pfs-AIC.csv")

##################------------surv rate calculate  ###  lthaz   ###---------------##############
lthaz_plot<-data.frame(ltHaz$Time,ltHaz[,11:29])
dfsurv_lthaz<-as.data.frame(array(dim=c(35,20)))
...
ggsave("fig_surv_che_pfs_fit.png",f_surv2,width = 12,height = 8,dpi = 600)
...
###
best<-flexsurvspline(Surv(recyrs, censrec) ~ 1, data = sd_bc, k = 5, scale = "hazard")
best$coefficients
