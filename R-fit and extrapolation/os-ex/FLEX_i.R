...
modFP2_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(1)) + I(Time^3) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^1) + I(Time^2) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
...
write.csv(dfGOF,"ser-os-AIC.csv")
...
ggsave("fig_surv_ser_os_fit.png",f_surv2,width = 12,height = 8,dpi = 600)
...
best<-flexsurvspline(Surv(recyrs, censrec) ~ 1, data = sd_bc, k = 2, scale = "hazard")
best$coefficients
